...
fig.suptitle("Task 3D", fontsize=16)
...
if stability_check(D0, dx, dt) > 0.5:
...
def update(frame):
    ...
    for j in range(1, M - 1):
        n[j] = n_old[j] + dt * D0 / dx**2 * (n_old[j+1] - 2 * n_old[j] + n_old[j-1]) - (1 + kmin) * dt * n_old[j]
        npos[j] = npos_old[j] - dt / dx * (npos_old[j]- npos_old[j-1]) + dt * n_old[j]
        nneg[j] = nneg_old[j] + dt / dx * (nneg_old[j+1] - nneg_old[j]) + dt * kmin * n_old[j]
...
# Save as GIF
anim.save("3dattachment30.gif", writer=PillowWriter(fps=30))
print("Animation saved as 3dattachment30.gif")
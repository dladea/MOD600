...
fig.suptitle("Task 3F", fontsize=16)
...
line_num, = ax[1].plot([], [], 'ob', label='Numerical Solution for n')
...
line_num_pos, = ax[2].plot([], [], 'ob', label='Numerical Solution for n+')
...
line_num_neg, = ax[3].plot([], [], 'ob', label='Numerical Solution for n-')
...
def update(frame):
    global n_old, n, npos_old, npos, nneg_old, nneg, flux
    ...
    for j in range(1, M - 1):
        ...
        npos[j] = npos_old[j] - dt / dx * (npos_old[j]- npos_old[j-1]) - dt * kp * npos_old[j] + dt * n_old[j]
        nneg[j] = nneg_old[j] + dt / dx * (nneg_old[j+1] - nneg_old[j]) - dt * kn * nneg_old[j] + dt * kmin * n_old[j]
...
# Save as GIF
anim.save("3ffull100.gif", writer=PillowWriter(fps=30))
print("Animation saved as 3ffull100.gif")

...
#line_analytical, = ax[2].plot([], [], '-g', label='Analytical Solution')

ax[3].set_ylabel("n negative")
ax[3].set_ylim(-0.01, 0.02)
...
def update(frame):
    global D0, n_old, n, npos_old, npos, nneg_old, nneg, flux, current_time #,  npos_analytical
    current_time = (frame+1) * dt
    for j in range(1, M - 1):
        n[j] = n_old[j] + dt * D0 / dx**2 * (n_old[j+1] - 2 * n_old[j] + n_old[j-1])
        npos[j] = npos_old[j] - dt / dx * (npos_old[j]- npos_old[j-1])
        nneg[j] = nneg_old[j] + dt / dx * (nneg_old[j+1] - nneg_old[j])
...
print("Animation saved as 3cwithoutsourceterm.gif")
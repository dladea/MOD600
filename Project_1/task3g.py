...
for a in range(11):
    ...
    def update(frame):
        global n_old, n, npos_old, npos, nneg_old, nneg
        current_time = (frame+1) * dt
        for j in range(1, M - 1):
            fxnposplus = 0.5 * (fx(npos_old[j], a) + fx(npos_old[j+1], a)) - 0.5 * (npos_old[j+1] - npos_old[j])
            fxnposmin = 0.5 * (fx(npos_old[j-1], a) + fx(npos_old[j], a)) - 0.5 * (npos_old[j] - npos_old[j-1])
            fxnnegplus = 0.5 * (fxminus(nneg_old[j], a) + fxminus(nneg_old[j+1], a)) - 0.5 * (nneg_old[j+1] - nneg_old[j])
            fxnnegmin = 0.5 * (fxminus(nneg_old[j-1], a) + fxminus(nneg_old[j], a)) - 0.5 * (nneg_old[j] - nneg_old[j-1])

            n[j] = n_old[j] + dt * D0 / dx**2 * (n_old[j+1] - 2 * n_old[j] + n_old[j-1]) + dt * (kp * npos_old[j] + kn * nneg_old[j]) - (1 + kmin) * dt * n_old[j]
            npos[j] = npos_old[j] - dt / dx * (fxnposplus - fxnposmin) - dt * kp * npos_old[j] + dt * n_old[j]
            nneg[j] = nneg_old[j] - dt / dx * (fxnnegplus - fxnnegmin) - dt * kn * nneg_old[j] + dt * kmin * n_old[j]
    ...
    # Save animation with a filename corresponding to the value of 'a'
    filename = f"3g_{a}.gif"
    anim.save(filename, writer=PillowWriter(fps=30))
    print(f"Animation saved as {filename}")

    # Restore initial values for the next iteration
    # Calculate numerical solution for free particles
    n_old = np.copy(n0)
    n = np.zeros(M)
